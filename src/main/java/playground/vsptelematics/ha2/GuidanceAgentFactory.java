...
public class GuidanceAgentFactory implements AgentFactory {
	...
	public MobsimAgent createMobsimAgentFromPerson(Person p) {
		...
		if (r < equipmentFraction){
			agent = new GuidanceWithindayAgent(p, this.simulation, this.guidance, timeInterpretation );
			this.ttObserver.addGuidedAgentId(p.getId());
		}
		else {
			agent = new PersonDriverAgentImpl(PopulationUtils.unmodifiablePlan(p.getSelectedPlan()), this.simulation, this.timeInterpretation );
